...
// Memtest variant: no video, so hclk/hclk5 do not exist in this netlist.
...
create_clock -name clk4x -period 3.367 -waveform {0 1.684} [get_nets {memory_clk}]
create_clock -name clk1x -period 13.47 -waveform {0 6.734} [get_nets {clk_x1}]
...
set_clock_groups -asynchronous -group [get_clocks {clk_sys}] -group [get_clocks {clk27}] -group [get_clocks {clk_g}] -group [get_clocks {clk4x}] -group [get_clocks {clk1x}]
