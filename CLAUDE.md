...
| `mcr123s_console60k/` | Tang Console 60K | GW5AT-LV60PG484 | **THE COMPLETE ROSTER: 15 games, four families (MCR-1 + MCR-2 + MCR-3 + MCR3Scroll) in one bitstream. Closes cleanly — 116/118 BSRAM, 75% logic, 0 setup/hold violations, clk_sys Fmax 43.0 vs 40.0 MHz. **FLASHED AS THE POWER-ON DEFAULT 2026-07-30** (`-f --verify`, clean readback). Five games confirmed on hardware so far: Wacko, Kick, Kickman, Solar Fox and Spy Hunter (with FX68K sound). Needed the build options above plus shared sprite ROM / scratch RAM across cores; see its `README.md` |
