...
function [7:0] idx_family(input [4:0] idx);
    idx_family = (idx < N_MCR3) ? FAM_MCR3 : FAM_MCR2;
endfunction
function [3:0] idx_slot(input [4:0] idx);
    idx_slot = (idx < N_MCR3) ? idx[3:0] : (idx[3:0] - N_MCR3[3:0]);
endfunction
...
wire dl_bg1_rng = dl_v2 && (ldr_is_mcr2 ? (dl_addr[17:13] == 5'b01110)
                                        : (dl_addr[17:14] == 4'b0101));
wire dl_bg2_rng = dl_v2 && (ldr_is_mcr2 ? (dl_addr[17:13] == 5'b01111)
                                        : (dl_addr[17:14] == 4'b0110));
wire dl_sp_rng  = (dl_v2 ? (dl_addr >= 18'h1C000) : 1'b1) && !ldr_is_mcr2;
...
always @(*) begin
    ...
    // both families' per-game maps live here, selected by the roster index.
    case (game_id)
    5'd2: begin
    ...
    default: ;   // roster indices beyond the 9 games
    endcase
    input_4 = 8'hFF;
end
...
wire       m2_reset = core_reset ||  run_is_mcr3;

wire [2:0]  m3_r, m3_g, m3_b, m2_r, m2_g, m2_b;
wire        m3_vblank, m3_hblank, m3_hs, m3_vs, m3_cs;
wire        m2_vblank, m2_hblank, m2_hs, m2_vs, m2_cs;
wire [9:0]  m3_hcnt, m3_vcnt, m2_hcnt, m2_vcnt;
wire [15:0] m3_al, m3_ar, m2_al, m2_ar;
wire [15:0] m3_rom_addr, m2_rom_addr;
wire [13:0] m3_snd_addr, m2_snd_addr;
wire [13:0] m3_bg_addr;
wire [12:0] m2_bg_addr;

assign r           = run_is_mcr3 ? m3_r        : m2_r;
assign g           = run_is_mcr3 ? m3_g        : m2_g;
assign b           = run_is_mcr3 ? m3_b        : m2_b;
assign vblank      = run_is_mcr3 ? m3_vblank   : m2_vblank;
assign hblank      = run_is_mcr3 ? m3_hblank   : m2_hblank;
assign hs          = run_is_mcr3 ? m3_hs       : m2_hs;
assign vs          = run_is_mcr3 ? m3_vs       : m2_vs;
assign cs          = run_is_mcr3 ? m3_cs       : m2_cs;
assign core_hcnt   = run_is_mcr3 ? m3_hcnt     : m2_hcnt;
assign core_vcnt   = run_is_mcr3 ? m3_vcnt     : m2_vcnt;
assign audio_l_val = run_is_mcr3 ? m3_al       : m2_al;
assign audio_r_val = run_is_mcr3 ? m3_ar       : m2_ar;
assign rom_addr    = run_is_mcr3 ? m3_rom_addr : m2_rom_addr;
assign snd_addr    = run_is_mcr3 ? m3_snd_addr : m2_snd_addr;
assign bg_addr     = run_is_mcr3 ? m3_bg_addr  : {1'b0, m2_bg_addr};
...
osd #(
    ...
    .NUM_GAMES(5'd9),
    .ROT_MASK(20'h00018),         // idx 3 shollow + idx 4 tron are ROT90
    .TITLE ("   MCR2/MCR3 SELECT     "),
    .NAME0 ("   TAPPER               "),
    .NAME1 ("   TIMBER               "),
    .NAME2 ("   DISCS OF TRON        "),
    .NAME3 ("   SATANS HOLLOW        "),
